...
// #define S2NCONFIG "s2ncfg" 
// #define S2NCONFIG_SECTION "com" 

// int s2nconfig_com_port = 8080;                    // 默认通信端口
// char s2nconfig_com_remoteip[32] = "127.0.0.1";   // 默认远程IP地址

// // UCI配置读取函数：从配置文件获取通信参数
// void get_s2ncfg_config()
// {   
...
// 	
// 	// 打开配置文件
// 	if(uci_load(uci_ctx, S2NCONFIG, &uci_pkg)){
...
// 	
// 	// 遍历配置文件中的所有section
// 	uci_foreach_element(&uci_pkg->sections, e1) {
// 		s = uci_to_section(e1);
// 		
// 		// 查找指定的section类型
// 		if (strcmp(s->type, S2NCONFIG_SECTION) == 0){
// 			// 读取通信模式配置
//             const char *mode = uci_lookup_option_string(uci_ctx, s, "mode");
//             if(mode)  
//                 MESSAGE_PRINT("s2nconfig com mode: %d\n", atoi(mode));    
// 
//             // 读取远程端口配置
//             const char *remoteport = uci_lookup_option_string(uci_ctx, s, "remoteport");
//             if(remoteport)  
//                 s2nconfig_com_port = atoi(remoteport);
//             else
//                 s2nconfig_com_port = 8080;  // 使用默认端口
//         
//             MESSAGE_PRINT("s2nconfig com port: %d\n", s2nconfig_com_port);           
// 		}
// 	}
// 	
// 	// 清理UCI资源
...
//     if (sock == -1) {
//         MESSAGE_PRINT("socket fail\n");
//         return -1;
//     }
// 
//     // 设置服务器地址结构
//     server_addr.sin_family = AF_INET;                          // IPv4地址族
//     server_addr.sin_port = htons(s2nconfig_com_port);          // 转换端口号为网络字节序
//     
//     // 将IP地址字符串转换为网络地址
//     if (inet_pton(AF_INET, s2nconfig_com_remoteip, &server_addr.sin_addr) <= 0) {
//         MESSAGE_PRINT("inet_pton fail\n");
//         close(sock);
//         return -1;
//     }
// 
...
// 
//     // 准备要发送的消息
//     strncpy(message,pacbuffer,strlen(pacbuffer));
//     
//     // 向服务器发送消息
